...
description: >
  操作手册与用户指南。用于生成、撰写、创建、导出操作手册、用户手册、使用说明、功能说明文档、快速入门，
  可基于需求文档和已有前端页面生成管理员或终端用户手册。
---
...
本 skill 已适配当前后台管理端项目启动框架，用于在 PRD 和原型之后生成交付物级操作手册。

| 端 | 输入 PRD | 输入原型 | 输出目录 |
|---|---|---|---|
| 后台管理端 | `PRD/系统名称/后台管理端/` | `原型/后台管理端/系统名称/` | `交付物/操作手册/后台管理端/` |

模块、功能、页面映射以 `PRD/系统名称/00-功能模块架构.md` 为准。
...
| 优先级 | 文件类型 | 查找方式 |
|--------|---------|---------|
| 最高 | 功能模块架构 | `PRD/系统名称/00-功能模块架构.md` |
| 高 | 后台管理端 PRD | `PRD/系统名称/后台管理端/**/*.md` |
| 中 | 后台管理端原型 | `原型/后台管理端/系统名称/**/*.html` |
| 低 | 设计规范 | `设计规范/*.md` |

扫描完成后直接基于内容工作，只询问一件事：**是否需要自动截图**（需要先启动本地原型服务）。
...
如需导出 Word 或 PDF，优先使用当前 Codex 文档/本地转换能力，将生成的 Markdown 手册转换到 `交付物/操作手册/`。

## 文档命名规范

`交付物/操作手册/{端}/{日期}-{项目名称}-{手册类型}-V{版本号}.md`

示例：`交付物/操作手册/后台管理端/20260330-智慧厂区巡检系统-管理员手册-V1.0.md`
