...
| 优先级 | 方案 | 能截弹窗 | 适用场景 |
|--------|------|---------|---------|
| ⭐ 最优 | Codex Browser 插件 | ✅ 能 | 最完整，可操作真实浏览器 |
| 次选 | Playwright | ❌ 静态页 | 跨平台，零配置 |
| 备选 | Puppeteer | ❌ 静态页 | 同上 |
| 兜底 | 本机浏览器（脚本自动探测） | ❌ 静态页 | 本机有 Chrome/Edge 等 |

## 方案 A：Codex Browser 模式（推荐）

Codex Browser 可直接控制真实浏览器，能截取弹窗、下拉菜单等动态内容。

截图指令格式：
```
请使用 Codex Browser 按以下步骤截图并保存到 交付物/操作手册/后台管理端/images/screenshots/：
...
生成截图计划文件 `交付物/操作手册/screenshot-plan.json`：
```json
{
  "baseUrl": "http://localhost:5173",
  "outputDir": "交付物/操作手册/后台管理端/images/screenshots",
  "loginUrl": "/login",
...
执行截图：
```bash
node .codex/skills/pm-operation-manual/scripts/screenshot.cjs --config 交付物/操作手册/screenshot-plan.json
```

> 注：如项目 package.json 包含 `"type": "module"`，必须使用 `.cjs` 版本。

截图保存到 `交付物/操作手册/后台管理端/images/screenshots/`。
...
| 弹窗未出现 | 需要先点击按钮 | 脚本模式不支持，改用 Codex Browser 或文字说明代替 |
