...
description: >
  图表/图示生成。用于生成、绘制或插入流程图、活动图、架构图、时序图、泳道图、ER图、UML图、组织架构图、BPMN、
  数据流图等；生成需求说明书中的图表时必须使用。
---
...
本 skill 已适配当前“按功能 PRD + 后台原型”项目启动框架，用于生成 PRD、原型说明、操作手册中需要的图表。
...
图表可被 PRD、操作手册或汇报材料引用。功能级 PRD 中如需图表，应优先引用 `交付物/图表/系统名称/` 下的图片，不在各功能目录重复维护多份图表。
...
```
1. 确定图表类型 → 查阅类型对照表
2. 读取对应模板 → examples/<类型>/*.xml
3. 基于模板结构编写 XML → 替换节点/连线内容
4. 保存 XML 源文件 → 交付物/图表/系统名称/src/<名称>.xml
...
```bash
$CODEX_HOME/skills/diagram-generator/scripts/render-diagram.sh <源文件.xml> <输出文件.png>
```
...
| 场景 | 模板目录 | 适用情况 |
|------|---------|---------|
| 流程图/活动图 | `examples/flowchart/` | 操作步骤、审批流程、用户操作流程 |
| 泳道图 | `examples/swimlane/` | 跨部门/跨角色协作流程 |
| 时序图 | `examples/sequence/` | 系统间交互、API 调用链 |
| UML 类图 | `examples/class/` | 数据模型、类关系 |
| 组织结构图 | `examples/orgchart/` | 人员架构、部门层级 |
| E-R 图 | `examples/er/` | 数据库表关系、实体关系 |
| 系统架构图 | `examples/architecture/` | 分层架构、技术栈 |
| BPMN | `examples/bpmn/` | 业务流程建模、订单流转 |

## 模板参考流程

1. 根据图表类型，读取 `examples/<类型>/` 下的模板文件
2. 理解模板的节点布局、连线方式、样式规范
3. 以模板为骨架，替换为实际业务内容
4. 保持模板的布局风格和配色方案
...
```
交付物/图表/系统名称/src/<模块>-<描述>.xml
```

示例：
- `交付物/图表/系统名称/src/login-flow.xml`
- `交付物/图表/系统名称/src/system-arch.xml`
- `交付物/图表/系统名称/src/order-sequence.xml`

### 输出文件（PNG/SVG）

```
交付物/图表/系统名称/<模块>-<描述>.png
```

示例：
- `交付物/图表/系统名称/login-flow.png`
- `交付物/图表/系统名称/system-arch.png`

### 文档引用

```markdown
![登录流程图](交付物/图表/系统名称/login-flow.png)
```
